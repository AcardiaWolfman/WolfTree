...
def traducirVozaTexto():
    ...
    with open('salser.wav', 'rb') as audio_file:
        ...
        speech_to_text.recognize_using_websocket(
            ...
            keywords=['ojos', 'cabello', 'sexo', "estatura", "piel", "nombre"],
...
class Servidor(object):
    ...
    def servirPorSiempre(self, socketTcp, listaconexiones):
        try:
            while True:
                ...
                thread_read.start()
                gestion_conexiones(self.listaConexiones)
    ...
    def recibir_datos(self, client_conn, client_addr):
        try:
            ...
            while True:
                ...
                recdata = client_conn.recv(self.buffer_size)
                voz.write(recdata)
                voz.close()
    ...
    if len(sys.argv) != 4:
        print("usage:", sys.argv[0], "<ip host> <puerto> <numero de conexiones>")
        sys.exit(1)
